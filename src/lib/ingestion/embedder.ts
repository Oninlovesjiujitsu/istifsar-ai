...
import { EMBEDDING_DIMENSIONS } from '@/src/lib/config/constants';
...
export async function embedQuery(query: string): Promise<string> {
  const { buildEmbeddingCacheKey, getCachedEmbedding, setCachedEmbedding } = await import('@/src/lib/cache/redis');
